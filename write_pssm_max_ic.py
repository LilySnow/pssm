...
usage: python write_pssm_max_ic.py <pssm file> <residueID list file>
example: python write_pssm_max_ic.py 1FT4.pssm resID.list
output:
      #resi    PSSMmax    PSSMic
      1    5    0.64
...
Last Modified: 2016-11-10 15:27:09 CET
"""
...
if __name__ == '__main__':
    ...
    resi = read_resi(fresi)
    # print resi
    pssmic = []
    if is_non_zero_file(fresi):
        print "#resi\tPSSMmax\tPSSMic"
        for i in resi:
            if i!="":
                if i in pssm.keys():
                    # print pssm[i]
                    # pssmic.append(float(pssm[i]['IC']))
                    out = "{}\t{}\t{:.2f}".format(i, pssm[i]['max'], float(pssm[i]['IC']))
                else:
                    sys.stderr.write("Error: {} resi {} not exist in {}!\n".format(fresi, i, fin))
                    sys.exit(1)
            else:
